...
import {  ArrowLeft, User  } from '@/utils/icons';
import { router, usePathname } from '@/utils/router';
...
export function AppHeader({ title, back, backTo }: AppHeaderProps) {
  ...
  return (
    <View style={[s.container, { paddingTop: topPad, backgroundColor: Platform.OS === 'ios' ? 'transparent' : colors.card, borderBottomColor: colors.border }]}>
      {Platform.OS === 'ios' && (
        <BlurView blurAmount={10} blurType={isDark ? 'dark' : 'light'} style={StyleSheet.absoluteFill} />
      )}
      <View style={s.row}>
        <View style={s.left}>
          {back ? (
