...
import {  Home, LayoutGrid, List, Bell, Layers, Activity, CircleMinus, CircleAlert, Radio, WifiOff, TriangleAlert, Database, ChevronUp, ChevronDown, ChevronLeft, ChevronRight, Search, Download, ExternalLink, Filter  } from '@/utils/icons';
...
export default function DTAnalyzerScreen() {
  ...
  const s = StyleSheet.create({
    ...
    tabRow: {
      ...
      paddingHorizontal: 8, paddingVertical: 10, paddingBottom: insets.bottom + 10, gap: 4,
    },
    tabBtn:       { flex: 1, flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingVertical: 4, borderRadius: 8, gap: 4 },
    tabBtnActive: { backgroundColor: colors.primary },
    tabLabel:     { fontSize: 10, fontFamily: 'Inter_600SemiBold', fontWeight: '600' as const, marginTop: 2 },
  });

  const contentPaddingTop = headerH + 4;
  ...
  return (
    <View style={s.container}>
      ...
      <View style={s.tabRow}>
        {([
        ...
        ] as { key: Tab | 'home'; label: string; icon: any }[]).map((t) => (
          <Pressable
            key={t.key}
          ...
          >
            <t.icon size={14} color={tab === t.key ? '#FFFFFF' : colors.mutedForeground} />
  ...
  const renderPagination = () => {
    ...
    return (
      <View style={s.paginationRow}>
        ...
        >
          <ChevronLeft size={14} color={alertPage === 0 ? colors.border : colors.foreground} />
        ...
        >
          ...
          <ChevronRight size={14} color={alertPage === totalPages - 1 ? colors.border : colors.foreground} />
        </Pressable>
      </View>
    );
  };
        ...
        <View style={s.toggleOuter}>
          ...
          <View>
            <Pressable style={s.filterIconBtn} onPress={() => setFilterOpen(true)}>
              <Filter size={17} color={activeFilterCount > 0 ? colors.primary : colors.foreground} />
            </Pressable>
            {activeFilterCount > 0 && (
        ...
        {dashMode === 'dt' ? (
          <>
            ...
            <View style={[s.kpiRow, { marginBottom: 16 }]}>
              <KpiCard label="Inactive DTs" value={filteredDtKpis.inactive} icon={CircleMinus}  warning />
              <KpiCard label="Under Outage" value={filteredDtKpis.outage}   icon={CircleAlert}  danger />
            </View>
            ...
            <View style={s.chartCard}>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 12, gap: 8 }}>
                ...
                {ratingView === 'graph' && (
                  <Download size={16} color={colors.mutedForeground} style={{ marginRight: 4 }} />
                )}
          ...
          <>
            ...
            <View style={s.kpiRow}>
              ...
              <KpiCard label="Inactive Sensors"  value={filteredSensorKpis.inactive}       icon={WifiOff}       warning />
            </View>
            <View style={[s.kpiRow, { marginBottom: 16 }]}>
              <KpiCard label="Critical Alarms"   value={filteredSensorKpis.criticalAlarms} icon={TriangleAlert} danger />
          ...
          pagedAlerts.map((a) => (
            <View key={a.id} style={s.alertRow}>
              ...
              >
                ...
                {expandedAlert === a.id ? <ChevronUp size={14} color={colors.mutedForeground} /> : <ChevronDown size={14} color={colors.mutedForeground} />}
              </Pressable>
              {expandedAlert === a.id && (
                <View style={s.alertDetail}>
                  ...
                  <Pressable
                    ...
                    onPress={() => router.push('dt-analyzer-detail', { code: a.dt, alertId: a.id })}
                  >
