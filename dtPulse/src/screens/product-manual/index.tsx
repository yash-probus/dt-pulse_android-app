...
import {  Zap, Droplets, Thermometer, Sun, CloudRain, Wifi, FileText, ChevronRight, Search, X  } from '@/utils/icons';
...
function iconForProduct(icon: string, color: string) {
  const map: Record<string, any> = {
    zap: Zap, droplet: Droplets, thermometer: Thermometer,
    sun: Sun, 'cloud-rain': CloudRain, wifi: Wifi,
  };
      ...
      <>
        ...
        {items.map((p) => (
          <Pressable
            ...
            onPress={() => router.push('product-manual-detail', { slug: p.slug })}
          >
    ...
    <View style={s.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll}>
        <View style={s.searchBox}>
          ...
          />
          {search ? <Pressable onPress={() => setSearch('')}><X size={16} color={colors.mutedForeground} /></Pressable> : null}
        </View>
