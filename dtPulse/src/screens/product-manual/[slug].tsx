...
import {  Zap, Droplets, Thermometer, Sun, CloudRain, Wifi, FileText, Info, ChevronUp, ChevronDown  } from '@/utils/icons';
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll}>
        <View style={s.headerCard}>
          <View style={[s.iconBox, { backgroundColor: `${manual.accent}18` }]}>
            <View>
              {(() => {
                const Icon = ({ zap: Zap, droplet: Droplets, thermometer: Thermometer, sun: Sun, 'cloud-rain': CloudRain, wifi: Wifi } as any)[manual.icon] ?? FileText;
                return <Icon size={28} color={manual.accent} />;
              })()}
            </View>
          </View>
        ...
        {manual.sections.map((section, i) => (
          <View key={i} style={s.sectionCard}>
            <Pressable style={s.sectionHeader} onPress={() => setOpenSection(openSection === i ? null : i)}>
              ...
              {openSection === i ? <ChevronUp size={16} color={colors.mutedForeground} /> : <ChevronDown size={16} color={colors.mutedForeground} />}
            </Pressable>
            {openSection === i && (
