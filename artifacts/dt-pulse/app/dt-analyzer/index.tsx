...
import {
  ...
  ACTIVE_ALERT_SUMMARY, ACTIVE_ALERTS, HIERARCHY,
  type ActiveAlert,
} from '@/lib/mockDT';
...
function DashboardTab({ paddingTop }: { paddingTop: number }) {
  ...
  const filteredAlerts = ACTIVE_ALERTS.filter((a) => {
    const matchSearch = !alertSearch || [a.dt, a.type, a.subDivision].some((v) => v.toLowerCase().includes(alertSearch.toLowerCase()));
  ...
  const s = StyleSheet.create({
    ...
    utilityCard: { backgroundColor: colors.card, borderRadius: 12, padding: 14, borderWidth: 1, borderColor: colors.border, flexDirection: 'row', alignItems: 'center', marginBottom: 16 },
    utilityLabel: { fontSize: 10, color: colors.mutedForeground, fontFamily: 'Inter_500Medium', letterSpacing: 0.5, textTransform: 'uppercase' as const },
    utilityValue: { fontSize: 16, fontWeight: '700' as const, color: colors.foreground, fontFamily: 'Inter_700Bold' },
    sectionTitle: { fontSize: 12, fontWeight: '700' as const, color: colors.foreground, fontFamily: 'Inter_700Bold', textTransform: 'uppercase' as const, letterSpacing: 1, marginBottom: 12 },
    ...
    selectBox: { flex: 1, height: 38, borderRadius: 8, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.card, paddingHorizontal: 10, justifyContent: 'center' },
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scroll}>
      ...
      <View style={s.utilityCard}>
        <View style={{ flex: 1 }}>
          <Text style={s.utilityLabel}>Utility</Text>
          <Text style={s.utilityValue}>KESCO, Kanpur</Text>
        </View>
        <Feather name="refresh-cw" size={18} color={colors.mutedForeground} style={{ marginRight: 12 }} />
        <Feather name="download" size={18} color={colors.mutedForeground} />
      </View>

      {/* Hierarchy */}
      <Text style={s.sectionTitle}>Hierarchy</Text>
      <View style={s.hierarchyRow}>
        {['Circle', 'Division', 'Sub-Div'].map((label, i) => (
          <View key={label} style={s.selectBox}>
            <Text style={s.selectText}>{label === 'Division' && !circle ? '—' : label === 'Sub-Div' && !division ? '—' : label}</Text>
          </View>
        ))}
      </View>

      {/* DT Overview */}
      <Text style={s.sectionTitle}>DT Overview</Text>
      <View style={s.kpiRow}>
      ...
      {/* Active Alert Summary KPI tiles */}
      ...
      {filteredAlerts.length === 0 ? (
        ...
        filteredAlerts.map((a) => (
          <Pressable key={a.id} style={s.alertRow} onPress={() => setExpandedAlert(expandedAlert === a.id ? null : a.id)}>
            ...
            {expandedAlert === a.id && (
              <View style={s.alertDetail}>
                {[
                  { label: 'Circle', value: a.circle }, { label: 'Division', value: a.division }, { label: 'Sub-Division', value: a.subDivision },
                  { label: 'Alert Type', value: a.type }, { label: 'Description', value: a.description },
                  { label: 'Alarm Value', value: a.alarmValue }, { label: 'Current Value', value: a.currentValue },
                  { label: 'Alarm Timestamp', value: a.alarmTs }, { label: 'Current Timestamp', value: a.currentTs },
                ].map((r) => (
